...
public class CrossviewHunterTarget : TargetRules
{
	public CrossviewHunterTarget(TargetInfo Target) : base(Target)
	{
		Type = TargetType.Game;

		ExtraModuleNames.AddRange(new string[] { "CrossviewHunter", "LyraGame" });
	...
	internal static void ApplySharedCrossviewHunterTargetSettings(TargetRules Target)
	{
		...
		if (Target.BuildEnvironment == TargetBuildEnvironment.Unique)
		{
			...
			CrossviewHunterTarget.ConfigureGameFeaturePlugins(Target);
		}
		else
		{
			// !!!!!!!!!!!! WARNING !!!!!!!!!!!!!
